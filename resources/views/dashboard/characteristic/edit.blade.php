        ...
        <form action="/dashboard/expert/characteristic/{{ $characteristics->id }}" method="POST" class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="kode" id="kode" class="bg-gray-100 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5" disabled value="{{ old('kode', $characteristics->kode) }}">
                </div>
                <div>
                    <label for="ciri" class="block mb-2 text-sm font-medium text-gray-900">Minat Bakat</label>
                    <input type="text" name="ciri" id="ciri" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5" required value="{{ old('ciri', $characteristics->ciri) }}">
                </div>
            </div>
