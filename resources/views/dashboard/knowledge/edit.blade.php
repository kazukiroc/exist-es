...
@section('content')
    <div class="p-12 sm:ml-64">
        <section class="bg-white mt-20 shadow-md rounded-lg max-w-md">
            <form action="/dashboard/expert/knowledge/{{ $intelligence->id }}" method="POST" class="relative bg-white rounded-lg shadow">
                @method('put')
                @csrf
                <div class="flex items-start justify-between p-4 border-b rounded-t">
                    ...
                    <a href="{{route('knowledge.index')}}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label for="prodi" class="block mb-2 text-sm font-medium text-gray-900">Minat Bakat</label>
                        <select data-te-select-init multiple name="character[]" id="character" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5" required>
                            @foreach ($characters as $character)
                                <option value="{{ $character->id }}" {{in_array($character->id, $knowledges->where('id_intelligence', $intelligence->id)->pluck('id_character')->toArray()) ? 'selected' : ''}}>{{ $character->kode . ' : ' . $character->ciri }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
