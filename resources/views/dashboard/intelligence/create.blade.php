...
<div class="p-12 sm:ml-64">
    <section class="bg-white mt-20 shadow-md rounded-lg max-w-md">
        <form action="/dashboard/expert/intelligence" method="POST" class="relative bg-white rounded-lg shadow">
            @csrf
            <div class="flex items-start justify-between p-4 border-b rounded-t">
                <h3 class="text-xl font-semibold text-gray-900">
                    Tambah Data
                </h3>
                <a href="/dashboard/expert/intelligence" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="kode" id="kode" value="{{ App\Models\Intelligence::generateKode() }}" class="bg-gray-100 border border-gray-300 text-gray-500 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5" readonly>
                </div>
                <div>
                    <label for="jenis" class="block mb-2 text-sm font-medium text-gray-900">Kategori Kecerdasan</label>
                    <input type="text" name="jenis" id="jenis" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5" required>
                </div>
{{--                <div>--}}
...
{{--                        <option value="{{ $study->nama }}">{{ $study->nama }}</option>--}}
{{--                        @endforeach--}}
{{--                    </select>--}}
{{--                </div>--}}
            </div>
